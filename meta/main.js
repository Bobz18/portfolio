...
function processCommits(data) {
  return d3
    ...
    .map(([commit, lines]) => {
      ...
      let ret = {
        id: commit,
        url: 'https://github.com/Bobz18/portfolio/commit/' + commit,
        author,
        date,
        time,
        timezone,
...
function renderCommitInfo(data, commits) {
  ...
  addStat('Average file length', d3.format('.1f')(averageFileLength));
}
...
function renderScatterPlot(data, commits) {
  ...
  yScale = d3
    ...
    .tickFormat((d) => {
        return d.getDate() <= 2 ? formatMonth(d) : formatDay(d);
    });
  const yAxis = d3
    .axisLeft(yScale)
